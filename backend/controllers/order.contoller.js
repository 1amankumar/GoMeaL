...
export const placeOrder = async (req, res) => {
    try {
        ...
        if (!deliveryAddress.text || !deliveryAddress.latitude || !deliveryAddress.longitude) {
            return res.status(400).json({ message: "send complete delivery adddress" })
        }
    ...
    try {
        ...
        if (status == 'out of delivery' && !shopOrder.assignment) {
            ...
            if (candidates.length == 0) {
                await order.save()
                return res.json({
                    message: "order status updated but there is no availabe delivery boys."
                })
            }
...
export const getOrderById = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: `getbyid order error ${error}` })
...
export const sendDeliveryOtp = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: `deliveryOtp error ${error}` })
    }
}
...
export const verifyDeliveryOtp = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: `verifyDelivery otp error ${error}` })
