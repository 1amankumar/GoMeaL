...
function DeliveryBoy() {
    ...
    const sendOtp = async () => {
        setLoading(true)
        try {
            const result = await axios.post(`${serverUrl}/api/order/send-delivery-otp`, { orderId: currentOrder.id, shopOrderId: currentOrder.shopOrder._id }, { withCredentials: true })
            setLoading(false)
            setShowOtpBox(true)
        } catch (error) {
            console.log(error)
    ...
    const verifyOtp = async () => {
        setMessage('')
        try {
            const result = await axios.post(`${serverUrl}/api/order/verify-delivery-otp`, { orderId: currentOrder.id, shopOrderId: currentOrder.shopOrder._id, otp }, { withCredentials: true })
            console.log(result.data)
    ...
    return (
        ...
        flex flex-col items-center gap-5 overflow-y-auto'>
            <Nav />
            <div className='w-full max-w-[800px] flex flex-col gap-5 items-center'>
                ...
                <div className='bg-white rounded-2xl shadow-md p-5 w-[90%] mb-6 border border-orange-100'>
                    ...
                    <ResponsiveContainer width='100%' height={200}>
                        <BarChart data={todayDeliveries}>
                            ...
                            <Tooltip formatter={(value) => [value, 'orders']} labelFormatter={(labal) => `${labal}:00`} />
                ...
                {!currentOrder && <div className='bg-white rounded-2xl p-5 shadow-md w-[90%] border border-orange-100'>
                    ...
                    <div className='space-y-4 '>
                        ...
                        ) : <p className='text-gray-400 text-sm'>Currently No Available Orders</p>}
                    </div>
                </div>}
                ...
                {currentOrder &&
                    <div className='bg-white rounded-2xl p-5 shadow-md w-[90%] border border-orange-100'>
                        ...
                        <div className='border rounde-lg p-4 mb-3'>
                        ...
                        {!showOtpBox ? <button className='mt-4 w-full bg-green-500 text-white font-semibold py-2 px-4 rounded-xl shadow-md hover:bg-green-600 active:scale-95 transition-all duration-200' onClick={sendOtp} disabled={loading}>
                            ...
                            <div className='mt-4 p-4 border rounded-xl bg-gray-50'>
                                ...
                                {message && <p className='text-center text-green-400 text-3xl'>{message}</p>}
