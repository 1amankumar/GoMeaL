    ...
    return (
        <div className='bg-white rounded-lg shadow p-4 space-y-4'>
            <div>
                ...
                <p className='flex items-center gap-2 text-gray-600 text-sm mt-1'><MdPhone color='green' size={16} /><span>{data.user.mobile}</span> </p>
                {data.paymentMethod == 'online' ? <p className='gap-2 text-gray-600'>Payment: {data.payment ? 'done✅' : 'pending⌛'}</p> : <p className='gap-2 text-gray-600'>Payment Method: {data.paymentMethod}</p>}
            ...
            <div className='flex justify-between items-center mt-auto pt-3 border-t border-gray-100'>
                ...
                <select className='rounded-md border px-3 py-1 text-sm focus:outline-none focus:ring-2 border-[#ff3d2d] text-[#ff4d2d]' onChange={(e) => handleUpdateStatus(data._id, data.shopOrders.shop._id, e.target.value)}>
...
//         <div className='bg-white rounded-lg shadow p-4 space-y-4'>
//             <div>
//                 <h2 className='text-lg font-semibold text-gray-800'>{data.user?.fullName || data.user?.name}</h2>
//                 <p className='text-sm text-gray-500'>{data.user?.email}</p>
//                 <p className='flex items-center gap-2 text-gray-600 text-sm mt-1'>
//                     <MdPhone color='green' size={16} />
...
//                     <div className='flex justify-between items-center mt-auto pt-3 border-t border-gray-100'>
//                         <span className='text-sm'>Status: <span>{shopOrder.status}</span></span>
//                     </div>
//                 </div>
//             ))}
//         </div>
